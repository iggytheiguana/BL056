...
class Contact extends CActiveRecord
{
    ...
    public static function createContact($params=array(),$model){
        $date = new DateTime();
        $user_settings = Settings::get_settings($params['who']); //user_id
        $contact = new Contact();
        ...
        if(!$model->message  and $user_settings){
            if($user_settings['with_message'] == 1){$contact->message = $user_settings['message'];}
        } else {
            $contact->message =  $model->message;}

        if(! $params['public_name']  and $user_settings){
           if($user_settings['with_pubname'] == 1){ $contact->public_name = $user_settings['public_name'];}
        } else{
