...
//        $parameters = array(":id"=>$contact_id);
        $query = "DELETE FROM `contact` WHERE user_id=:user_id and contact_id=:contact_id";
        $parameters = array(':user_id' => $chatData['user_id'], ':contact_id' => $chatData['contact_id']);
        $command = Yii::app()->db->createCommand($query);
        $command->execute($parameters);
        
        $query = "DELETE FROM `contact` WHERE user_id=:user_id and contact_id=:contact_id";
        $parameters = array(':user_id' => $chatData['contact_id'], ':contact_id' => $chatData['user_id']);
        $command = Yii::app()->db->createCommand($query);
        $command->execute($parameters);
...
//        }
        ...
        if($is_search == 1)
        {
            ...
            {
                $sql = "insert into favorite (user_id, chat_id) values (:user_id, :chat_id)";
                $parameters = array(":user_id"=>$user_id, ':chat_id' => $chat_id);
                Yii::app()->db->createCommand($sql)->execute($parameters);
                
                Helper::set_favorite_push('Set favorite', $user_id, $chat_id);
            }
        }
...
//        }
    ...
    public static function lookup() {
        ...
        {
            $searchResult = Yii::app()->db->createCommand()
            ->select("c.*,GROUP_CONCAT(ct.tag) As tags",false)
            ->from('chat c')
            ->leftJoin('chat_tag ct', 'ct.chat_id = c.id')
            ->where('c.title LIKE :match AND c.type = '.$type.' AND c.is_searchable = 1 or ct.tag LIKE :match AND c.type = '.$type.' AND c.is_searchable = 1', array(':match' => "%$param_serch%"))
            ->group('c.id')
            ->limit($limit)
            ->offset($offset)
            ->queryAll();
        ...
        if(!empty($searchResult))
        {
            foreach($searchResult as $key => $result)
            {
                ...
                ->from('favorite')
                ->where('chat_id=:chat_id', array(':chat_id'=>$result['id']))
                ->queryRow();

                $number_of_favorites = $favoriteListData['count'];
                
                $searchResult[$key]['number_of_likes'] = $number_of_favorites;
                $searchResult[$key]['is_favorite'] = $is_favorite;
                
                $offset = Yii::app()->getRequest()->getPost('page', 0);
                $limit = Yii::app()->getRequest()->getPost('limit', 5);
                ...
                $query->params=array(':chat_id'=>$result['id']);
                ...
                $searchResult[$key]['messages'] = $messagesData;
            }
        }
