...
//            if(!empty($userIds))
//            {
                Helper::set_chat_push('Chat updated', $user_id, $chat_id, $tagString);
//            }
        }
        else
        {
...
//                    {
                        Helper::set_chat_push('Chat updated', $user_id, $chat_id, $tagString);
                        //Helper::set_chat_push('Chat updated', $userIds, $chat_id);
//                    }
    ...
    public static function contact_remove() {
        ...
        ->select('id')
        ->from('user')
        ->where('qrcode=:qrcode', array(':qrcode'=>$qrcode))
        ->queryRow();
        
        if (!$user) {
            static::_send_resp(null, 802, 'user not found');
        }
        $user_id = $user['id'];
        $contact_id = Yii::app()->getRequest()->getPost('contact_id');
...
//        $res1->delete();
    ...
    public static function chat_add_member() {
        ...
        if(!empty($userIds))
        {
            ...
            Helper::chat_add_member_push('Chat add member', $to_add['id'], $chat_id, $numberOfMembers, $chat['type']);
        }
...
//        fclose($fp);
    ...
    public static function setFavorite() {
        ...
        if(!empty($userIds))
        {
            Helper::set_favorite_push('Set favorite', $userIds, $chatID);
        }
    ...
    public static function setFlagged() {
        ...
        if ($messageData) 
        {
            ...
            if(!empty($userIds))
            {
                Helper::set_flagged_push('Set flagged', $userIds, $messageID, $numberOfFlagged['count']);
            }
