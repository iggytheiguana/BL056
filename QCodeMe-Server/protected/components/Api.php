...
Class Api extends ApiBase {
    ...
    public static function account_create() {
        ...
        $UUID = Yii::app()->getRequest()->getPost('UUID');
        if($password == '')
    ...
    public static function account_login()
    {
        ...
        {
            $user->UUID = $data['UUID'];
            $user->auth_token = Helper::generate_token($user->getPrimaryKey());
            $user->last_active = new CDbExpression('NOW()');
            $user->last_login = new CDbExpression('NOW()');
            if($user->save()){
                Helper::sent_registration_push('Account was token',$user->id); // add push
            }else{static::_send_resp($user->getErrors());}
        }
    ...
    public static function accountRenewToken(){

        static::_check_auth();
        $reqUUID = Yii::app()->getRequest()->getPost('UUID');
        $user = User::model()->findByPk(static::$_user_id);
        if($user->UUID !== $reqUUID){static::_send_resp(null, 203, 'Account was taken.');}
