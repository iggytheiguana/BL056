...
class SyncAdapter extends AbstractThreadedSyncAdapter {
	 ...
	 * {@link android.content.AbstractThreadedSyncAdapter} guarantees that this
	 * will be called on a non-UI thread, so it is safe to peform blocking I/O
	 * here.
	 * 
	 * <p>
	...
	public void onPerformSync(Account account, Bundle extras, String authority,
		...
		final boolean afterLogin = extras.getBoolean(SyncHelper.SYNC_EXTRAS_AFTER_LOGIN, false);

		if (afterLogin) {
			SyncHelper.doInitialSync(mContext, mContentResolver);
		} else {
			// Sync contacts
			SyncHelper.doContactsSync(mContext, mContentResolver);
			// Sync chats
			// SyncHelper.doChatSync(mContext, mContentResolver);
			SyncHelper.doFromPreference(mContext, mContentResolver);
			// Sync user settings
			SyncHelper.doSettingsSync(mContext);
			// Sync Messages
			SyncHelper.doMessageSync(mContext, mContentResolver);
			// Sync Chats favorite
			SyncHelper.doChatFavoriteSync(mContext, mContentResolver);
		}
