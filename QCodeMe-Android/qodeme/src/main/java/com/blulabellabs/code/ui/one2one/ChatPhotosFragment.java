...
public class ChatPhotosFragment extends Fragment {
    ...
    public static ChatPhotosFragment newInstance(Contact c) {
    ...
    public void updateUi() {
        if (isViewCreated && callback != null) {
            ...
            if (messages != null) {
                ...
                List<PhotoMessage> arrayListMessage = new ArrayList<ChatPhotosFragment.PhotoMessage>();
                for (Message me : temp) {
                        ...
                        try {
                            ...
                            if (currentDate.get(Calendar.DATE) != previousDate.get(Calendar.DATE)) {
                                ...
                                if (arrayListMessage.size() > 0) {
                                    PhotoMessage message = arrayListMessage.get(arrayListMessage.size() - 1);
                ...
                for (PhotoMessage photoMessage : arrayListMessage) {
                    List<HashMap<String, Message>> mMapList = new ArrayList<HashMap<String, Message>>();
                    ...
                    mListAdapter.addSection(photoMessage.date, new GridAdapter(getActivity(),
                            (ArrayList<HashMap<String, Message>>) mMapList, mImageFetcher));
                }
                mListViewPhotos.setAdapter(mListAdapter);
            }
        }
