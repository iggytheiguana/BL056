	...
	public static List<ChatLoad> getChatList(Cursor cursor) {
		...
		if (cursor.moveToFirst())
			do {
				...
				c.tag = cursor.getString(QodemeContract.Chats.ChatQuery.CHAT_TAGS);
				...
				if (memQr != null && !memQr.trim().equals("")) {
					c.members = memQr.split(",");
				}
