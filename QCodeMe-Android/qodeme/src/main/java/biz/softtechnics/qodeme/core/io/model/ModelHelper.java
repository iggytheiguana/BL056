...
public class ModelHelper {
    ...
    public static List<Contact> getContactList(Cursor cursor){
        ...
        if (cursor.moveToFirst())
            do {
                ...
                c.publicName = cursor.getString(QodemeContract.Contacts.ContactQuery.CONTACT_PUBLIC_NAME);
                c.location = cursor.getString(QodemeContract.Contacts.ContactQuery.CONTACT_LOCATION);
                contactList.add(c);
    ...
    public static class MessageStructure{
        ...
        public Map<Long, Long> getLastMessageInChatMap() { return lastMessageInChatMap; }

        public void setLastMessageInChatMap(Map<Long, Long> lastMessageInChatMap) { this.lastMessageInChatMap = lastMessageInChatMap; }
    }
        ...
        if (cursor.moveToFirst())
            do {
                ...
                if (m.state == QodemeContract.Messages.State.NOT_READ){
                    ...
                    if (timeMarker == null || (timeMarker + TimeUnit.MINUTES.toMillis(1)) < createdLong){
    ...
    private static void removeTimeMarkerFoeMessage(List<Message> messages, Message mL) {
        for (int i = messages.size()-1; i >= 0; i-- ){
            ...
            if (!TextUtils.isEmpty(created)) {
                ...
                if (tLl == null || tLl ==  null || Math.abs(tLl - tRl) > TimeUnit.MINUTES.toMillis(1)){
                    return;
                } else {
                    if (mL._id > mR._id){
                        mL.created = "";
